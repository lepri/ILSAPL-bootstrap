...
if [ -z $senha ]; then
    ...
    mysql -h 127.0.0.1 -u $usuario interlegis < $NEW_INST/Products/ILSAPL/instalacao/sapl_migracao_banco.sql
else
    mysql -h 127.0.0.1 -u $usuario interlegis --password=$senha < $NEW_INST/old/sapl_old.sql
    mysql -h 127.0.0.1 -u $usuario interlegis --password=$senha < $NEW_INST/Products/ILSAPL/instalacao/sapl_migracao_banco.sql
fi
if [ $? -gt 0 ]; then
    echo ""
